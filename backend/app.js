...
function hashit(beforehash) {
  var crypto = require('crypto');
  var hash = crypto.createHash('sha256');
  data = hash.update(beforehash, 'utf-8');
  afterhash = data.digest('hex');
  return afterhash;
}
...
app.get('/checkIfSchedExists', (req, res) => {
  let params = req.query;
  let id = params.schedule;
  let statement = `SELECT * FROM Schedule WHERE id = ${id}`;
  console.log(statement);
...
app.get('/checkIfSchedCollides', (req, res) => {
  let params = req.query;
  let id = params.id;
  let day= params.day;
  let statement = `SELECT * FROM Schedule WHERE id = ${id} and day="${day}"`;
  console.log(statement);
...
app.get('/makeAccount', (req, res) => {
  ...
  let sql_statement = `INSERT INTO Patient (email, password, name, address, gender,phno) 
                       VALUES ` + `("${email}", "${password}", "${name}", "${address}", "${gender}","${phno}")`;
  console.log(sql_statement);
  con.query(sql_statement, function (error, results, fields) {
    if (error) throw error;
    else {
...
app.get('/createSchedules', (req, res) => {
  let params = req.query;
  let id = params.id;
  ...
  let day = params.day;
  let sql_statement = `INSERT INTO Schedule (id,starttime,endtime,breaktime,day) 
...
app.post('/resetPasswordPatient', (req, res) => {
  ...
  oldPassword = hashit(oldPassword);
  let newPassword = "" + something.newPassword;
  newPassword = hashit(newPassword);
...
app.get('/checkIfApptExists', (req, res) => {
  ...
  con.query(statement, function (error, results, fields) {
    if (error) throw error;
      ...
      WHERE doctor="${doc_email}" AND date=${sql_date} AND status="NotDone"
      AND ((${sql_start} >= starttime AND ${sql_start} < endtime)
      OR (ADDTIME(${sql_start}, "1:00") > starttime AND ADDTIME(${sql_start}, "1:00") <endtime));
      `
      console.log(statement)
      con.query(statement, function (error, results, fields) {
        if (error) throw error;
          ...
          day=DAYNAME(${sql_date}) AND ${sql_start} >= starttime AND 
          (ADDTIME(${sql_start}, "1:00") <= endtime) AND
          (ADDTIME(${sql_start}, "1:00") <= breaktime OR ${sql_start} >= ADDTIME(breaktime, "1:00"));
          `
...
app.get('/OneHistory', (req, res) => {
  ...
  let statement = `SELECT gender,name,email,address,conditions,surgeries,medication,phno
